...
And here is the quiet punchline. Nothing about that recipe cared that the data was three tidy blobs. Scatter points across any shape, drop a small Gaussian on each, and the score is *still* the committee of leashes, just with more members voting. That is exactly how the heart at the top of this page was made. The target here is not a single picture but a *shape*: the distribution of points lying on a heart. About eleven hundred Gaussians, scattered over that silhouette, define it, and their exact closed-form score is the committee of leashes. Pour in noise, walk it back twice, stochastic and deterministic, and each grain is pulled onto the shape; the swarm *is* the heart. No neural network anywhere; just Einstein's jittering grains, reversed, drawing a valentine.

So why train one? Here the leap to real images matters. On the heart, each Gaussian sat on a *point* of the shape; for photographs, the thing that plays the role of a point is an entire image. A megapixel photo is a single dot in a space with millions of axes, one per pixel, and the dataset is a vast cloud of such dots. To model it as a mixture you would place one Gaussian on every training photo; with billions of photos that is billions of bumps, and the score sums over all of them at each step. Hopeless. Worse, it can only pull a sample back toward an image it has already seen; it memorizes, it cannot invent. A network, a smooth approximation with limited capacity, is *forced* to interpolate between training images instead, and that interpolation is what making a new image actually is. Its imperfection is the whole point.

Everything else is the century of physics you have just walked through. Only that one function, the learned stand-in for a score too expensive to write down, is new.
